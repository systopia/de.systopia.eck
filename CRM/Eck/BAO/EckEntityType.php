...
class CRM_Eck_BAO_EckEntityType extends CRM_Eck_DAO_EckEntityType implements HookInterface {
  ...
  public static function ensureEntityType(array $entity_type): void {
    ...
    $subtypeRequired = (bool) ($entity_type['has_subtypes'] ?? FALSE);
    $subtypeConstraint = $subtypeRequired ? 'NOT NULL' : 'DEFAULT NULL';
    ...
    CRM_Core_DAO::executeQuery("
      CREATE TABLE IF NOT EXISTS `{$table_name}` (
          ...
          `subtype` text $subtypeConstraint COMMENT 'The entity subtype.',
  ...
  public static function getSubTypes($entity_type_name, $as_mapping = TRUE): array {
    $result = OptionValue::get(FALSE)
      ->addSelect('value', 'name', 'label', 'description', 'icon')
  ...
  public static function self_hook_civicrm_pre(PreEvent $event): void {
    ...
    switch ($event->action) {
      ...
      case 'edit':
        ...
        // When changing the has_subtypes property, the schema must be updated
        if (isset($event->params['has_subtypes'])) {
          $sql = 'SELECT has_subtypes FROM civicrm_eck_entity_type WHERE id = %1';
          $previousHasSubtypes = (bool) CRM_Core_DAO::singleValueQuery($sql, [
            1 => [$event->id, 'Integer'],
          ]);
          $newHasSubtypes = (bool) $event->params['has_subtypes'];
          self::onChangeHasSubtypes($eckTypeName, $previousHasSubtypes, $newHasSubtypes);
        }
        break;
  ...
  public static function self_hook_civicrm_post(PostEvent $event): void {
    ...
    // Shouldn't this be part of the metadata cache clear?
    unset(\Civi::$statics['civi.entity.fields']);
  ...
  public static function onChangeHasSubtypes(string $eckTypeName, bool $oldValue, bool $newValue): void {
    if ($oldValue !== $newValue) {
      $fieldDefn = \Civi::entity('Eck_' . $eckTypeName)->getField('subtype');
      $fieldDefn['required'] = $newValue;
      E::schema()->alterSchemaField('Eck_' . $eckTypeName, 'subtype', $fieldDefn, NULL);
      // Delete all subtypes if disabled
      if (!$newValue) {
        $table_name = _eck_get_table_name($eckTypeName);
        CRM_Core_DAO::executeQuery("UPDATE `$table_name` SET subtype = NULL");
        OptionValue::delete(FALSE)
          ->addWhere('option_group_id:name', '=', 'eck_sub_types')
          ->addWhere('grouping', '=', $eckTypeName)
          ->execute();
