...
<table>
  ...
  {foreach from=$entity_types item=entity_type key=entity_type_id}
    ...
    <tr>
      ...
      <td><ul>
          <li><a href="{crmURL p='civicrm/admin/eck/entity-type' q="reset=1&action=update&type=$entity_type_name"}">{ts}Edit{/ts}</a></li>
          <li><a href="{crmURL p='civicrm/admin/eck/entity-type' q="reset=1&action=delete&type=$entity_type_name"}">{ts}Delete{/ts}</a></li>
        </ul>
      </td>
    </tr>
  {/foreach}
